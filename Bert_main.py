...
tf.app.flags.DEFINE_string('bert_base_dir', 'D:/RODATA/IIITN/LABS/NLP/ECPE-2D/bert/uncased_L-12_H-768_A-12/', 'base dir of pretrained bert')
...
def build_maintask_CR(s_emo, s_cause, pred_emo_feature, pred_cause_feature, pos_embedding, doc_len):
    ...
    ##
    tmp = tf.cast(list(range(FLAGS.max_doc_len)), tf.int32)
...
def build_model(word_embedding, pos_embedding, x_bert, sen_len_bert, x, sen_len, doc_len, is_training):
    x = tf.nn.embedding_lookup(word_embedding, x)
    x = tf.reshape(x, [-1, FLAGS.max_sen_len, FLAGS.embedding_dim])
    keep_prob = is_training * FLAGS.keep_prob1 + (1.-is_training)
...
def get_batch_data(x_bert, sen_len_bert, x, sen_len, doc_len, is_training, y_emotion, y_cause, y_pair, batch_size, test=False):
    for index in batch_index(len(y_cause), batch_size, test):
        feed_list = [x_bert[index], sen_len_bert[index], x[index], sen_len[index], doc_len[index], is_training, y_emotion[index], y_cause[index], y_pair[index]]
...
def run():
    ...
    print('build model...')
    x_bert = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_doc_len, FLAGS.max_sen_len_bert]) 
    sen_len_bert = tf.compat.v1.placeholder(tf.int32, [None, FLAGS.max_doc_len]) 
    ...
    y_emotion = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.max_doc_len, FLAGS.n_class])
    y_cause = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.max_doc_len, FLAGS.n_class])
    if FLAGS.trans_type=='cross_road':
        y_pair = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.max_doc_len * FLAGS.max_doc_len, FLAGS.n_class])
    else:
        y_pair = tf.compat.v1.placeholder(tf.float32, [None, FLAGS.max_doc_len * (FLAGS.window_size*2+1), FLAGS.n_class])
    placeholders = [x_bert, sen_len_bert, x, sen_len, doc_len, is_training, y_emotion, y_cause, y_pair]  
    
    pred_emo, pred_cause, pred_pair, reg = build_model(word_embedding, pos_embedding, x_bert, sen_len_bert, x, sen_len, doc_len, is_training)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for fold in range(1,11):
            ...
            if FLAGS.trans_type=='cross_road':
                tr_doc_id, tr_y_emotion, tr_y_cause, tr_y_pair, tr_y_pairs, tr_x_bert, tr_sen_len_bert, tr_x, tr_sen_len, tr_doc_len = load_data_CR_Bert('data/'+train_file_name, tokenizer, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len_bert, FLAGS.max_sen_len)
                te_doc_id, te_y_emotion, te_y_cause, te_y_pair, te_y_pairs, te_x_bert, te_sen_len_bert, te_x, te_sen_len, te_doc_len = load_data_CR_Bert('data/'+test_file_name, tokenizer, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len_bert, FLAGS.max_sen_len)
            else:
                tr_doc_id, tr_y_emotion, tr_y_cause, tr_y_pair, tr_y_pairs, tr_x_bert, tr_sen_len_bert, tr_x, tr_sen_len, tr_doc_len, tr_pair_left_cnt = load_data_WC_Bert('data/'+train_file_name, tokenizer, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len_bert, FLAGS.max_sen_len, window_size = FLAGS.window_size)
                te_doc_id, te_y_emotion, te_y_cause, te_y_pair, te_y_pairs, te_x_bert, te_sen_len_bert, te_x, te_sen_len, te_doc_len, te_pair_left_cnt = load_data_WC_Bert('data/'+test_file_name, tokenizer, word_id_mapping, FLAGS.max_doc_len, FLAGS.max_sen_len_bert, FLAGS.max_sen_len, window_size = FLAGS.window_size)
            ...
            for i in xrange(FLAGS.training_iter):
                ...
                def combine_result(x):
                    ...
                    return np.array(ret) 
                # train                        
                for train, _ in get_batch_data(tr_x_bert, tr_sen_len_bert, tr_x, tr_sen_len, tr_doc_len, 1, tr_y_emotion, tr_y_cause, tr_y_pair, FLAGS.batch_size):
                    ...
                    if step % 10 == 0:
                        print('step {}: train loss {:.4f} run_lr {:.6f}'.format(step, loss, run_lr_tmp))
                    ...
                    test_results.append([pred_y_cause, true_y_cause, pred_y_emo, true_y_emo, pred_y_pair, true_y_pair, doc_len_batch])
                    if step % 60 == 0:
                        test_results = zip(*test_results)
                        pred_y_cause, true_y_cause, pred_y_emo, true_y_emo, pred_y_pair, true_y_pair, doc_len_batch = map(combine_result, test_results)
                        test_results = []
                        ...
                        if FLAGS.trans_type=='cross_road':
                            ...
                            p, r, f1 = pair_prf_WC(pred_y_pair, true_y_pair, doc_len_batch, threshold = FLAGS.threshold, window_size =FLAGS.window_size)
                        print('pair_prediction: train p {:.4f} r {:.4f} f1 {:.4f}\n'.format(p, r, f1 ))
                    step = step + 1
                # test
                test_results = []
                for test, _ in get_batch_data(te_x_bert, te_sen_len_bert, te_x, te_sen_len, te_doc_len, 0., te_y_emotion, te_y_cause, te_y_pair, FLAGS.batch_size, test=True):
